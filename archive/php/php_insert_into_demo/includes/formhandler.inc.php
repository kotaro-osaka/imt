...
if ($_SERVER["REQUEST_METHOD"] == "POST") { // Check for post request receipt
	...
	try {
		...
		// 1. Positional params
		$query = "INSERT INTO users (username, pwd, email) VALUES (?, ?, ?);"; // Values inserted later

		$stmt = $pdo->prepare($query); // Prepare query for post submission of data as statement

		$stmt->execute([$username, $pwd, $email]); // Execute prepared statement obj with assigned args to params
